...
namespace ge {
	...
	public:
		...
		void addElement(ge::Vector2<unsigned> position, T&& element);

		//Delete an element at a given position
		void deleteElement(ge::Vector2<unsigned> position);

		//Check if an element exist
		bool elementExist(ge::Vector2<unsigned> position) const { return vector.size() > position.x && vector[position.x].size() > position.y && isValid(vector[position.x][position.y]); }

		//Get the element at the given position. Does not check if the element exist
		T& operator[](ge::Vector2<unsigned> position) { return vector[position.x][position.y]; }
		const T& operator[](ge::Vector2<unsigned> position) const { return vector[position.x][position.y]; }

		class Iterator {
		private:
			VectorOfVector<T>* vectorOfVector;
			ge::Vector2<unsigned> position{ 0, 0 };
			bool hasEnded{ false };
		public:
			//Constructor
			Iterator(VectorOfVector<T>* vectorOfVector, ge::Vector2<unsigned> startPosition = { 0, 0 }) : vectorOfVector{ vectorOfVector }, position{ startPosition } {
			...
			Vector2<unsigned> getPosition() const{ return position; }
		};
	...
	template <typename T>
	void VectorOfVector<T>::addElement(ge::Vector2<unsigned> position, T&& element) {
	...
	template <typename T>
	void VectorOfVector<T>::deleteElement(ge::Vector2<unsigned> position) {
